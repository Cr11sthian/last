document.addEventListener("DOMContentLoaded", function () {
    ...
    // Función para agregar dinámicamente la lista
    function agregarLista() {
        const lista = document.getElementById("lista");

        cosasQueAmo.forEach((cosa) => {
            const listItem = document.createElement("li");
            listItem.innerHTML = `${cosa.numero}. ${cosa.texto}`;
            lista.appendChild(listItem);
        });
    }

    // Verificar si la lista está vacía antes de agregar elementos
    if (document.getElementById("lista").childElementCount === 0) {
        // Llama a la función para agregar la lista solo si está vacía
        agregarLista();
    }
});
